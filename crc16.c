
#include "crc16.h"
#include <linux/types.h>

static const uint8_t auchCRCHi[256] = {  };
static const char auchCRCLo[256] = {  };

uint16_t CRC16(uint16_t oldCRC, uint8_t val) {
	uint8_t uIndex = 0;
	uint8_t CRCH = (oldCRC >> 8);
	uint8_t CRCL = (oldCRC & 0xff);

	uIndex = CRCL ^ val;
	CRCL = CRCH ^ auchCRCHi[uIndex];
	CRCH = auchCRCLo[uIndex];

	return ((uint16_t) CRCH << 8) + CRCL;
}

uint16_t CRC16_buf(uint16_t oldCRC, uint8_t *val, uint16_t DataLen) {
	uint8_t uIndex = 0;
	uint8_t CRCH = (oldCRC >> 8);
	uint8_t CRCL = (oldCRC & 0xff);

	while (DataLen--) {
		uIndex = CRCL ^ *val++;
		CRCL = CRCH ^ auchCRCHi[uIndex];
		CRCH = auchCRCLo[uIndex];
	}
	return ((uint16_t) CRCH << 8) + CRCL;
}

uint16_t CRC16short(uint8_t *Msg, uint16_t DataLen) {
	volatile uint8_t CRCH = 0xFF;
	volatile uint8_t CRCL = 0xFF;
	volatile uint8_t uIndex = 0;

	while (DataLen--) {
		uIndex = CRCL ^ *Msg++;
		CRCL = CRCH ^ auchCRCHi[uIndex];
		CRCH = auchCRCLo[uIndex];
	}

	return ((uint16_t) CRCH << 8) + CRCL;
}

uint8_t CRC16m(uint8_t *Msg, uint16_t DataLen) {
	volatile uint8_t CRCH = 0xFF;
	volatile uint8_t CRCL = 0xFF;
	volatile uint8_t uIndex = 0;

	if (DataLen < 3) {
		return 0;
	}

	DataLen -= 2;
	while (DataLen--) {
		uIndex = CRCL ^ *Msg++;
		CRCL = CRCH ^ auchCRCHi[uIndex];
		CRCH = auchCRCLo[uIndex];
	}

	return (*Msg == CRCL && *(Msg + 1) == CRCH);
}

EXPORT_SYMBOL(CRC16);
EXPORT_SYMBOL(CRC16_buf);
EXPORT_SYMBOL(CRC16m);
EXPORT_SYMBOL(CRC16short);
